...
uint64_t unwrap(WrappingInt32 n, WrappingInt32 isn, uint64_t checkpoint) {
    uint32_t offset_n = n - isn;    // Absolute sequence number of n but with wrapping(32 bits)
    uint32_t offset_check = (checkpoint) % (1ll << 32); // Absolute sequence number of checkpoint but with wrapping(32 bits)
